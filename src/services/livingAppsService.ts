...
import type { Kunden, Material, Mitarbeiter, Auftraege, CreateKunden, CreateMaterial, CreateMitarbeiter, CreateAuftraege } from '@/types/app';
...
export class LivingAppsService {
  ...
  static async getMaterialEntry(id: string): Promise<Material | undefined> {
    ...
    const record = { record_id: data.id, ...data } as Material;
    return enrichLookupFields([record], 'material')[0];
  }
  static async createMaterialEntry(fields: CreateMaterial) {
    return callApi('POST', `/apps/${APP_IDS.MATERIAL}/records`, { fields: cleanFieldsForApi(fields as any, 'material') });
  }
  static async updateMaterialEntry(id: string, fields: Partial<CreateMaterial>) {
    return callApi('PATCH', `/apps/${APP_IDS.MATERIAL}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'material') });
  }
  ...
  static async getAuftraegeEntry(id: string): Promise<Auftraege | undefined> {
    ...
    const record = { record_id: data.id, ...data } as Auftraege;
    return enrichLookupFields([record], 'auftraege')[0];
  }
  static async createAuftraegeEntry(fields: CreateAuftraege) {
    return callApi('POST', `/apps/${APP_IDS.AUFTRAEGE}/records`, { fields: cleanFieldsForApi(fields as any, 'auftraege') });
  }
  static async updateAuftraegeEntry(id: string, fields: Partial<CreateAuftraege>) {
    return callApi('PATCH', `/apps/${APP_IDS.AUFTRAEGE}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'auftraege') });
  }
